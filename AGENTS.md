...
- When an LTP test takes a long time, distinguish tests whose design is inherently slow from tests that are slow because the kernel is blocking, retrying, timing out, or otherwise behaving incorrectly. Check the LTP source and the relevant kernel path before treating a long runtime as acceptable.
...
- When investigating `CONF` or `SKIP` LTP results, verify whether the skip is genuinely expected for the configured environment or whether a kernel bug is making LTP believe a missing feature, unsupported configuration, or normal precondition failure occurred.
- When collecting LTP failures to fix, include non-expected `SKIP` results in the repair list, especially skips caused by missing kernel features, missing kconfig exposure, missing syscalls, missing namespace/procfs support, unsupported protocol families, or incomplete filesystem behavior. Only exclude skips that are explicitly expected for the current target, such as unsupported compat16 coverage, architecture-specific unavailable syscalls, or intentionally unsupported features documented for that run.
...
- Avoid free helper functions of the form `fn x(y: T);` when the behavior belongs to `T`; prefer inherent methods such as `impl T { fn x(&self /* ... */); }` so behavior stays attached to the owning type.
...
- If some behavior, feature, edge case, or compatibility detail is intentionally left unimplemented for now, add an explicit `TODO` or `FIXME` comment at the relevant code location explaining what is missing so it is not forgotten later.
...
- Kernel unit tests are not automatically the source of truth. When a kernel unit assertion disagrees with implementation behavior and the Linux ABI oracle is not already proven, verify the expected Linux behavior on a real Linux system or upstream source before changing implementation code to satisfy the unit.
...
- Keep MCP VM launches headless by default. Do not enable `display` unless the task explicitly needs a visible UI or a screenshot, because this environment may not have a usable video device.
