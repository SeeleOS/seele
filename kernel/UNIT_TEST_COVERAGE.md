...
| `misc/framebuffer_ioctl.rs` | `not worth testing` | Linux fbdev ioctl structs and request-number constants are plain ABI declarations consumed by higher-level fbdev request routing and runtime behavior. |
| `misc/framebuffer.rs`, `fb_object.rs` | `integration-only` | Real framebuffer mapping, scanout presentation, fbdev ioctl state, and user-controlled display switching depend on live boot framebuffer memory and active process mappings. |
...
| `terminal/color.rs`, `state.rs`, `term_trait.rs`, `flanterm/alloc.rs`, `flanterm/mod.rs` | `not worth testing` | Type aliases, global holders, trait declarations, allocator shims, and module re-exports do not contain independent kernel behavior. |
| `terminal/macros.rs`, `impls.rs`, `mod.rs`, `object.rs`, `object_config.rs`, `linux_vt.rs`, `pty/*`, `flanterm/callback.rs`, `flanterm/terminal.rs` | `integration-only` | Terminal printing, tty ioctl handling, VT activation, PTY wakeups, flanterm callbacks, and framebuffer-backed terminal output all depend on live tty/display/runtime object state. Pure ioctl-number decoding is covered through `object/config.rs` tests. |
...
| `evdev/mod.rs` | `not worth testing` | Module wiring and public re-exports do not add behavior beyond the covered leaf modules. |
| `evdev/object.rs`, `queue.rs`, `ps2.rs` | `integration-only` | Queue wakeups, revoke/grab behavior, client/hub lifecycle, and PS/2 packet delivery all depend on live input/runtime object state even though some pure metadata helpers are covered elsewhere. |
| `drm/mode.rs` | `unit-tested` | DRM fourcc values, default mode constants, and grouped flag unions are covered. |
| `drm/card.rs` | `not worth testing` | Static DRM object ids and device-number constants are ABI/data declarations used by higher-level runtime paths. |
| `drm/client.rs` | `not worth testing` | DRM ioctl numbers, capability constants, event flags, and C ABI structs are declarations rather than executable behavior. |
...
| `drm/mode_types.rs` | `not worth testing` | DRM mode/KMS ABI structs are plain layout declarations; their semantics are exercised through higher-level ioctl routing and runtime handlers. |
| `drm/mod.rs` | `not worth testing` | Module wiring and the legacy scanout re-export do not add standalone behavior. |
| `drm/state.rs`, `object.rs`, `configure.rs`, `user.rs`, `fs.rs`, `events.rs`, `allocation.rs`, `framebuffer.rs`, `*_handlers.rs` | `integration-only` | Typed request routing is covered indirectly through `object/config.rs` tests, but DRM object IO, sysfs/devfs exposure, event delivery, dumb-buffer allocation, scanout, and handler execution still depend on live display/buffer/process state. |
