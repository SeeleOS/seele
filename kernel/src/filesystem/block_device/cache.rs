...
impl CachedBlockDevice {
    ...
    fn take_eviction_candidate(state: &mut CacheState) -> Option<(usize, CacheEntry)> {
        while let Some(block_id) = state.lru.pop_front() {
            if let Some(entry) = state.entries.remove(&block_id) {
                return Some((block_id, entry));
            }
        }
    ...
    fn ensure_capacity(&self) -> Result<(), BlockDeviceError> {
        loop {
            let eviction = {
                ...
                Self::take_eviction_candidate(&mut state)
            };

            let Some((block_id, entry)) = eviction else {
                return Ok(());
            };
    ...
    fn collect_dirty_runs(&self) -> Vec<(usize, Vec<u8>)> {
    ...
    fn mark_clean_range(&self, start_block: usize, block_count: usize) {
        let mut state = self.state.lock();
        for block_id in start_block..start_block + block_count {
...
impl BlockDevice for CachedBlockDevice {
    ...
    fn flush(&self) -> Result<(), BlockDeviceError> {
        for (start_block, buffer) in self.collect_dirty_runs() {
            let block_count = buffer.len() / self.block_size();
            self.inner.write_blocks(start_block, &buffer)?;
            self.mark_clean_range(start_block, block_count);
        }
        self.inner.flush()
    }
}
