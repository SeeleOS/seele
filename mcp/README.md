...
6. Run `cargo xvm-ps` and verify no runner/QEMU process remains.

If the MCP server is unavailable, use `cargo xrun -- --agent` and `cargo xvm-ps` as the fallback path.
